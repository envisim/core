...
const binomialCoefsFun = (a: number, b: number): number => {
  return (
    (13860.0 - (462.0 - (132.0 - (99.0 - 140.0 / a) / a) / a) / a) /
    b /
    166320.0
  );
};

interface IRandomBinomialLarge {
  upperTail: boolean;
  n0: number;
  p: number;
  q: number;
...
interface IRandomBinomialSmall {
  upperTail: boolean;
  n0: number;
  frac: number;
  delta: number;
...
const randomBinomialLarge = (
  ...
  const constants: IRandomBinomialLarge = {
    upperTail,
    n0,
    p,
    q,
...
const randomBinomialLarge_inner = (
  ...
  while (run++ <= 1e5) {
    ...
    // BTPE 5.2
    rho = (k / npq) * ((k * (k / 3 + 0.625) + FRAC1_6) / npq + 0.5);
    t = (-k * k) / (2 * npq);
    A = Math.log(u);
...
const randomBinomialSmall = (
  ...
  const constants: IRandomBinomialSmall = {
    upperTail,
    n0,
    frac,
    delta,
