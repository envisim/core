import {TCollectionType} from './types/collection.js';

export function getCollectionType(
  geojson: GeoJSON.GeoJSON,
  checkCircle: boolean = false,
): TCollectionType | '' {
  switch (geojson.type) {
    case 'FeatureCollection':
      return geojson.features.length > 0
        ? getCollectionType(geojson.features[0])
        : '';

    case 'Feature':
      if (!geojson.geometry) return '';
      // Handle circle special case
      const tp = getCollectionType(geojson.geometry);
      return checkCircle && tp === 'point' && geojson.properties?._radius > 0.0
        ? 'polygon'
        : tp;

    case 'GeometryCollection':
      return geojson.geometries.length > 0
        ? getCollectionType(geojson.geometries[0])
        : '';

    case 'LineString':
    case 'MultiLineString':
      return 'line';

    case 'Point':
    case 'MultiPoint':
      return 'point';

    case 'Polygon':
    case 'MultiPolygon':
      return 'polygon';

    default:
      throw new Error('not a valid geometry');
  }
}

export function getCollectionTypeWithRadius(
  geojson: GeoJSON.GeoJSON,
  radiusIsDefined: boolean = false,
): TCollectionType | '' {
  const t = getCollectionType(geojson);
  return t === 'point' && radiusIsDefined === true ? 'polygon' : t;
}
