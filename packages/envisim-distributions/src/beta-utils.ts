...
export class BetaParams {
  static DEFAULTS = { alpha: 1.0, beta: 1.0 } as const;
  #alpha: number;
  #beta: number;
  ...
  #betaFunction?: number;
  betaFunction(): number {
    return this.#betaFunction ?? (this.#betaFunction = Math.exp(this.logBetaFunction()));
  }

  #logBetaFunction?: number;
  logBetaFunction(): number {
    return (
      this.#logBetaFunction ??
      (this.#logBetaFunction =
        logGammaFunction(this.#alpha) +
        logGammaFunction(this.#beta) -
        logGammaFunction(this.#alpha + this.#beta))
    );
  }
  ...
  regularizedBetaFunction(
    x: number,
    eps: number = 1e-20,
    logbetafn: number = this.logBetaFunction(),
  ): number {
    ...
    const cx = 1.0 - x;
    if (alpha < psq * x)
      return 1.0 - new BetaParams(beta, alpha).regularizedBetaFunction(cx, eps, logbetafn);
  ...
  incompleteBetaFunction(
    ...
    return Math.exp(logbetafn) * this.regularizedBetaFunction(x, eps, logbetafn);
  }
  ...
  inverseRegularizedBetaFunction(
    ...
    if (p > 0.5)
      return (
        1.0 - new BetaParams(beta, alpha).inverseRegularizedBetaFunction(1.0 - p, eps, logbetafn)
      );
    ...
    outer: while (run1++ <= 1e5) {
      // 7
      y = this.regularizedBetaFunction(ret, eps, logbetafn);
   ...
   * ISBN: 9780521430647
   */
  betaContinuedFraction(x: number, eps: number = 1e-20): number {
    const { alpha, beta } = this;
