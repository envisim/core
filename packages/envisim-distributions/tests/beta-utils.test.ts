...
describe('beta-utils', () => {
  test('beta', () => {
    ...
    const b = x1.map((e, i) => betaFunction({alpha: e, beta: x2[i]}));
    const lb = x1.map((e, i) => logBetaFunction({alpha: e, beta: x2[i]}));
    const res = [
  ...
  test('regularized beta', () => {
    ...
    const rb = x.map((e) => regularizedBetaFunction(e, {alpha: 4, beta: 2}, EPS));
    const ib = x.map((e) => incompleteBetaFunction(e, {alpha: 4, beta: 2}, EPS));
    const res = [
    ...
    const ibres = res.map((e) => e * betaFunction({alpha: 4, beta: 2}));
  ...
  test('inverse reg beta', () => {
    const x = fromTo(0, 10, 1).map((e) => e / 10);
    const rb = x.map((e) => inverseRegularizedBetaFunction(e, {alpha: 4, beta: 2}, EPS));
    const res = [
