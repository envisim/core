...
import { Pivotal } from "../src/sampling-classes/index";
...
test("lpm1Int", () => {
  const lpmInt = new Pivotal(Pivotal.LPM1, 2, data, 10, 2, 1e-9, rand01);
...
test("lpm2Int", () => {
  const lpmInt = new Pivotal(Pivotal.LPM2, 2, data, 10, 2, 1e-9, rand01);
  ...
  expect(lpm2({ n: 2, auxiliaries: data, rand: rand01, treeBucketSize: 2 })).arrayToEqual([1, 3]);
});
...
test("rpm", () => {
  expect(rpm({ probabilities: new Array<number>(4).fill(0.5), rand: rand01 })).arrayToEqual([1, 3]);
});

test("spm", () => {
  expect(spm({ probabilities: new Array<number>(4).fill(0.5), rand: rand01 })).arrayToEqual([1, 3]);
});
